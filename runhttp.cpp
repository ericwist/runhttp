...
int main(int argc, char* argv[])
{
    ...
    if (argc < 3)
    {
        cerr << "usage: " << argv[0] << " <server-address> " <<
            "<docroot> "
            "[server-port] [indexFileName]" << endl << endl;
        cerr << "\tIf you don't pass server-port, it defaults to " <<
            defaultServerPort << ", indexFileName defaults to index.html." << endl;
        return 1;
    }
    ...
    cout << "fileAttrs= " << fileAttrs << " FILE_ATTRIBUTE_DIRECTORY= " << FILE_ATTRIBUTE_DIRECTORY <<
        " (fileAttrs & FILE_ATTRIBUTE_DIRECTORY)= " << (fileAttrs & FILE_ATTRIBUTE_DIRECTORY) << endl;

    if (fileAttrs == 0xFFFFFFFF)
    {
        cerr << "GetFileAttributes() FAILED! GetLastError()= " << GetLastError() << endl << endl;
    }

    if ((fileAttrs == 0xFFFFFFFF) || !(fileAttrs & FILE_ATTRIBUTE_DIRECTORY))
    {
        cerr << "Error: " << dirName << " does not exist as a directory" << endl << endl;
        return 1;
    }
