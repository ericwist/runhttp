...
HttpCookie::HttpCookie(const char& name, const char& value) : comment(NULL), domain(NULL),
	path(NULL), maxAge(-1), version(0), secure(FALSE), next(NULL)
{
...
HttpCookie::HttpCookie(HttpCookie& cookie) : comment(NULL), domain(NULL),
	path(NULL), value(NULL), maxAge(cookie.getMaxAge()), version(cookie.getVersion()),
	secure(cookie.getSecure()), next(NULL)
{
	if ( cookie.name )
	{
